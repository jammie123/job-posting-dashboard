...
import { Heading1, Heading2, Heading3, List, Bold, Italic, ListOrdered, AlignLeft, Sparkles, Loader2, Maximize2, X } from "lucide-react"
...
<ul class="list-disc pl-5 my-2">
  ...
  if (isExpanded) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div className="w-[90%] h-[90%] bg-white rounded-lg shadow-2xl flex flex-col animate-in zoom-in-95 duration-300">
          {/* Header */}
          <div className="flex items-center justify-between p-6 border-b">
            <h2 className="text-xl font-semibold">Popis pozice - Celá obrazovka</h2>
            <Button
              variant="ghost"
              size="icon"
          ...
          <div className="flex-1 p-6 overflow-auto">
            {value ? (
              <div 
                className="text-base description-content prose prose-lg max-w-none"
                dangerouslySetInnerHTML={{ __html: sanitizedValue }} 
              />
            ) : (
              <p className="text-lg text-muted-foreground">Nespecifikováno</p>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="flex justify-between items-start mr-2 w-full h-full">
          ...
          <div className="flex gap-1 absolute right-0 top-0 hidden group-hover:flex transition-opacity">
            ...
            <Button
              ...
              size="icon"
              onClick={toggleExpand}
              className="h-7 w-7 p-1 rounded-full hover:bg-green-100 hover:text-green-700 group"
              title="Rozbalit na celou obrazovku"
            >
              <Maximize2 className="h-4 w-4" />
              <span className="sr-only">Rozbalit na celou obrazovku</span>
              <div className="absolute hidden group-hover:block right-0 top-full mt-1 bg-black text-xs p-1.5 rounded whitespace-nowrap z-50 text-white">
                Rozbalit na celou obrazovku
              </div>
            </Button>
  ...
  return (
    <div className={`${blurClass} flex flex-col h-full gap-4`}>
