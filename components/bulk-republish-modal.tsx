...
interface BulkRepublishModalProps {
...
export function BulkRepublishModal({
  selectedJobs = [],
  trigger,
  open,
  onOpenChange,
  onConfirm,
}: BulkRepublishModalProps) {
  ...
  // Extrahuji jedinečné portály a počítám inzeráty pro každý portál (pouze z neaktivních inzerátů)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-[800px] w-full">
        <DialogHeader>
          <DialogTitle>Hromadně znovu vystavit inzeráty ({inactiveJobs.length} náborů)</DialogTitle>
        </DialogHeader>
        <div className="py-4">
          {activeJobs.length > 0 && (
            <Alert variant="destructive" className="mb-4 bg-red-500/10 border-red-500">
              ...
              <AlertDescription>
                U {activeJobs.length} {activeJobs.length === 1 ? "náboru" : "náborů"} nelze obnovit inzerci, protože {activeJobs.length === 1 ? "je již aktivní" : "jsou již aktivní"}:
          ...
          {inactiveJobs.length === 0 ? (
            <p className="text-center py-4">Nebyly nalezeny žádné neaktivní inzeráty, které by bylo možné obnovit.</p>
          ) : (
            <>
        ...
        <DialogFooter>
          ...
          <Button 
            ...
            disabled={selectedPortals.length === 0 || inactiveJobs.length === 0}
          >
            Znovu vystavit na {selectedPortals.length} {selectedPortals.length === 1 ? "portálu" : "portálech"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
