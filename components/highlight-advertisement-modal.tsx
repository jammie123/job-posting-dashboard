...
interface HighlightAdvertisementModalProps {
  portals?: JobPortal[]
...
export function HighlightAdvertisementModal({
  portals = [],
  trigger,
  open,
  onOpenChange,
  onConfirm,
  job,
}: HighlightAdvertisementModalProps) {
  ...
  // Seznam portálů, které podporují zvýrazňování
  const supportedPortals = ["jobs.cz", "prace.cz", "prace za rohem"]
  
  // Filtrování portálů, které podporují zvýrazňování
  ...
  // Mapování názvu portálu na produkty pro zvýrazňování
  const getHighlightProductsForPortal = (portalName: string) => {
    const normalizedPortalName = portalName.toLowerCase()
    
    return mockProducts.filter(product => {
      const productPortalName = product.namePortal.toLowerCase()
      
      // Kontrola, jestli produkt patří k správnému portálu a je pro zvýrazňování
      return (
        productPortalName.includes(normalizedPortalName) && 
        (product.name.toLowerCase().includes("zvýraznění") || 
         product.name.toLowerCase().includes("supermax") || 
         product.name.toLowerCase().includes("jobtips"))
      )
    })
  }
  ...
  const handleConfirm = async () => {
    ...
    if (job && job.advertisement && job.advertisement.portals) {
      try {
        ...
        toast({
          title: "Zvýraznění úspěšně přidáno",
          description: "Zvýraznění inzerátu bylo úspěšně uloženo.",
        })
        ...
        toast({
          title: "Chyba při zvýrazňování",
          description: "Nepodařilo se uložit zvýraznění inzerátu.",
          variant: "destructive",
        })
      }
    }
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        <DialogContent className="sm:max-w-[800px] w-full">
          <DialogHeader>
            <DialogTitle>Zvýraznit inzerát</DialogTitle>
          </DialogHeader>
          <div className="py-4">
            <p className="text-sm text-muted-foreground mb-10">
              Zvýraznění zajistí, že váš inzerát bude více viditelný a přitáhne větší pozornost potenciálních kandidátů. <a href="#" className="text-blue-500 hover:underline">Více informací o zvýrazňování v nápovědě</a>
            </p>
            
            <div className="space-y-4">
              {filteredPortals.length === 0 ? (
                <div className="text-center py-8 px-4 border rounded-lg bg-muted/20">
                  <p className="text-muted-foreground">Nemáte žádné aktivní portály, které podporují zvýrazňování inzerátů.</p>
                </div>
              ) : (
                filteredPortals.map((portal) => {
                  const highlightProducts = getHighlightProductsForPortal(portal.name)
                  const portalHasProducts = highlightProducts.length > 0
                        ...
                        <Card>
                          <CardContent className="py-6 text-center">
                            <p className="text-muted-foreground">
                              Pro tento portál nejsou dostupné služby zvýrazňování.
                            </p>
                          </CardContent>
                        </Card>
                      )}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleConfirm} disabled={selectedProducts.length === 0}>
              Zvýraznit {selectedProducts.length > 0 ? `(${selectedProducts.length})` : ""}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
